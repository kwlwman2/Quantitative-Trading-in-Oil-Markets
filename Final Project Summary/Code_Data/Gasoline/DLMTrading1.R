...
#setwd("G:/Save_D/Dokumente/Energy_Futures/Gasoline")
#source("Code.R")

assets<-c("RBC1","RBC5");

crude_prices<-data[,c("Date","RBC1","RBC5")];
crude_prices[,1]<-as.Date(crude_prices[,1],format="%d.%m.%Y")

#Specify the lookback period for rolling mean and rolling sd
lb<-200;

#Data only available since 3/10/2005
dates<-c("2005-01-01","2006-01-01","2007-01-01","2008-01-01",
...
for(i in (offset+1):length(crude_prices$Date)){
	...
	outMLE <- dlmMLE(dlmSample[,2],parm=rep(0,3),buildHedge,
		method = c("BFGS"),control=
		list(trace=1, REPORT = 10, maxit = 50))
